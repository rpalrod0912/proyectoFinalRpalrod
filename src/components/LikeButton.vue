<template>
  <div @click="likedProduct()" class="heartContainer">
    <input @click="likedProduct()" type="checkbox" id="checkbox" />
    <label for="checkbox">
      <svg
        id="heart-svg"
...
export default {
  mounted() {
    ...
    if (list !== null) {
      const productInList = list.wishList.findIndex((string) => {
  ...
  methods: {
    likedProduct() {
      ...
      if (productAlreadyInList !== -1) {
        document.querySelector("#checkbox").checked = false;
        list.wishList.splice(productAlreadyInList, 1);
      } else {
        document.querySelector("#checkbox").checked = true;
        list.wishList.push(this.productName);
      }
...
@import "../styles/components/LikeButton.scss";
